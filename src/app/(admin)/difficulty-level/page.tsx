...
export default function DifficultyLevelPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return levels.filter((x) => !q || x.name.toLowerCase().includes(q));
  }, [levels, query]);
  ...
  const submit = async (values: FormValues) => {
    if (editing?.id) {
      ...
      if (!error && data) setLevels((prev) => [data, ...prev]);
    }
    setOpen(false);
  };
  ...
  const onDelete = async () => {
    ...
    if (!error)
      setLevels((prev) => prev.filter((p) => p.id !== toDelete.id));
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Niveles de dificultad</h1>
            <p className="text-muted-foreground">
              Catálogo para clasificar la dificultad de los entrenamientos.
            </p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Define el nombre que identifica al nivel de dificultad.
                </DialogDescription>
              </DialogHeader>
              <form
                className="space-y-4"
